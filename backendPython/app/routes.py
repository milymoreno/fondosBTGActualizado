...
from services import  cancelar_suscripcion, obtener_historial_transacciones, get_cliente_por_identificacion,  get_cliente_por_id, get_todas_las_transacciones
from models import add_cliente, get_cliente, update_cliente, delete_cliente, add_transaccion, add_fondo, get_fondos, get_fondo
...
def route_suscribir_fondo(cliente_id):
    fondos = request.json
    cliente_actualizado = (cliente_id, fondos)
...
@app.route('/clientes/<cliente_id>/transacciones', methods=['POST'])
def route_registrar_transaccion(cliente_id):
    transacciones = request.json
    cliente_actualizado = add_transaccion(cliente_id, transacciones)
...
def route_cancelar_suscripciones(cliente_id):
    ...
    # Llamar a la función cancelar_suscripcion y capturar su retorno como una tupla
